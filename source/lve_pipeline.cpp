...
namespace lve {
    ...
    lve_pipeline::lve_pipeline(lve_Device &device,
                               std::string const &vert_filepath,
                               std::string const &frag_filepath,
                               pipeline_config_info const &configInfo) : lve_device{device} {
        create_graphics_pipeline(vert_filepath, frag_filepath, configInfo);
    }

    lve_pipeline::~lve_pipeline() {
        vkDestroyShaderModule(lve_device.device(), vertShaderModule, nullptr);
        vkDestroyShaderModule(lve_device.device(), fragShaderModule, nullptr);
    ...
    std::vector<char> lve_pipeline::read_file(std::string const &filepath) {
    ...
    void lve_pipeline::create_graphics_pipeline(std::string const &vert_filepath,
                                                std::string const &frag_filepath,
                                                pipeline_config_info const &configInfo) {

        /*assert(
                configInfo.pipeline_layout != VK_NULL_HANDLE &&
        "Cannot create graphics pipeline: no pipeline_layout provided in config_info");*/

        /*assert(
                configInfo.render_pass != VK_NULL_HANDLE &&
        ...
        create_shader_module(vert_code, &vertShaderModule);
        create_shader_module(frag_code, &fragShaderModule);
        ...
        pipeline_info.pInputAssemblyState = &configInfo.input_assembly_info;
        pipeline_info.pViewportState = &configInfo.viewport_info;
        pipeline_info.pRasterizationState = &configInfo.rasterization_info;
        pipeline_info.pMultisampleState = &configInfo.multisample_info;
        pipeline_info.pColorBlendState = &configInfo.color_blend_info;
        pipeline_info.pDepthStencilState = &configInfo.depth_stencil_info;
        pipeline_info.pDynamicState = nullptr;

        pipeline_info.layout = configInfo.pipeline_layout;
        pipeline_info.renderPass = configInfo.render_pass;
        pipeline_info.subpass = configInfo.subpass;

        pipeline_info.basePipelineIndex = -1;
        pipeline_info.basePipelineHandle = VK_NULL_HANDLE;

        if(vkCreateGraphicsPipelines(lve_device.device(),
                                     VK_NULL_HANDLE,
                                     1,
                                     &pipeline_info,
    ...
    void lve_pipeline::create_shader_module(const std::vector<char> &code, VkShaderModule *shader_module) {
        VkShaderModuleCreateInfo createInfo{};
        createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        createInfo.codeSize = code.size();
        createInfo.pCode = reinterpret_cast<const uint32_t*>(code.data());

        if(vkCreateShaderModule(lve_device.device(), &createInfo, nullptr, shader_module) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create shader module");
        }
    }

    void pipeline_config_info::init_default_pipeline_config_info(uint32_t width, uint32_t height, pipeline_config_info &config_info) {
